...
void r_2_1024_128_16384_128_16384_128_16384_128_256_2_2(float* restrict data0_2, float* restrict data1_2, float* restrict data2_1024, float* restrict data3_128, float* restrict data4_16384, float* restrict data5_128, float* restrict data6_16384, float* restrict data7_128, float* restrict data8_16384, float* restrict data9_128, float* restrict data10_256, float* restrict data11_2) {
  ...
  *(buf3+0) = 0.0f;
  for (int Ridx5 = 0; Ridx5 < 128; Ridx5++) {
    float val4 = (*(data5_128+Ridx5));
    *(buf3+0) = ((*(buf3+0))+(val4*val4));
  }
  float buf4[1];
  *(buf4+0) = 0.0f;
