...
void r_16384_1024_8_16384_128_16384_128_16384_128_256_2_2_4_4(float* restrict data0_16384, float* restrict data2_1024, float* restrict data3_128, float* restrict data4_16384, float* restrict data5_128, float* restrict data1_16384, float* restrict data6_128, float* restrict data7_16384, float* restrict data8_128, float* restrict data9_256, float* restrict data10_2, float* restrict data11_2) {
  float buf0[1];
  *(buf0+0) = 0.0f;
  float buf1[1];
  *(buf1+0) = 0.0f;
  ...
  *(buf10+0) = 0.0f;
  ...
  for (int Ridx2 = 0; Ridx2 < 8; Ridx2++) {
    int alu13 = (Ridx2<<4);
    float4 val1 = (*((float4*)((data3_128+alu13))));
    float4 val2 = (*((float4*)((data3_128+(alu13+4)))));
    float4 val3 = (*((float4*)((data3_128+(alu13+8)))));
    float4 val4 = (*((float4*)((data3_128+(alu13+12)))));
    *(buf9+0) = ((*(buf9+0))+(val1[0]*val1[0])+(val1[1]*val1[1])+(val1[2]*val1[2])+(val1[3]*val1[3])+(val2[0]*val2[0])+(val2[1]*val2[1])+(val2[2]*val2[2])+(val2[3]*val2[3])+(val3[0]*val3[0])+(val3[1]*val3[1])+(val3[2]*val3[2])+(val3[3]*val3[3])+(val4[0]*val4[0])+(val4[1]*val4[1])+(val4[2]*val4[2])+(val4[3]*val4[3]));
  }
  for (int Ridx3 = 0; Ridx3 < 16384; Ridx3++) {
    float val5 = (*(data4_16384+Ridx3));
    *(buf8+0) = ((*(buf8+0))+(val5*val5));
  }
  for (int Ridx5 = 0; Ridx5 < 128; Ridx5++) {
    float val6 = (*(data5_128+Ridx5));
    *(buf7+0) = ((*(buf7+0))+(val6*val6));
  }
  for (int Ridx6 = 0; Ridx6 < 16384; Ridx6++) {
    float val7 = (*(data1_16384+Ridx6));
  ...
  for (int Lidx16 = 0; Lidx16 < 16384; Lidx16++) {
    float val14 = (*(data1_16384+Lidx16));
    ...
    *(data0_16384+Lidx16) = (val14*alu35);
  }
}
